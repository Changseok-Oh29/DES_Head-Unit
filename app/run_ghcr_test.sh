...
# Default image tag (can be overridden with environment variable)
IMAGE_TAG="${IMAGE_TAG:-latest}"

echo "═══════════════════════════════════════════════════"
echo "PDC Docker Integration Test (GHCR Images)"
...
# Step 8: Allow X11 access for Docker
...
# Step 9: Pull and run containerized apps from GHCR
echo "Pulling and starting containerized apps from GHCR..."
echo "────────────────────────────────────────────────────"
cd "$APP_DIR"
...
IMAGE_TAG=$IMAGE_TAG docker-compose -f docker-compose.ghcr.yml pull

echo ""
echo "🚀 Starting all apps..."
if ! IMAGE_TAG=$IMAGE_TAG docker-compose -f docker-compose.ghcr.yml up -d; then
...
echo ""
echo "📝 Commands:"
echo "   View all logs:          docker-compose -f docker-compose.ghcr.yml logs -f"
echo "   View HU_MainApp logs:   docker logs -f humainapp"
echo "   View GearApp logs:      docker logs -f gearapp"
echo "   Stop all:               ./stop_ghcr_test.sh"
echo ""
