...
int main(int argc, char *argv[])
{
    ...
    if (qgetenv("VSOMEIP_APPLICATION_NAME").isEmpty()) {
        qputenv("VSOMEIP_APPLICATION_NAME", "ICGearApp");
    }

    if (qgetenv("VSOMEIP_CONFIGURATION").isEmpty()) {
        qputenv("VSOMEIP_CONFIGURATION", "/etc/vsomeip/vsomeip_icgear.json");
    }
    ...
    app.setApplicationName("ICGearApp");
    app.setApplicationVersion("1.0");
    app.setOrganizationName("SEA-ME");

    qDebug() << "═══════════════════════════════════════════════════════";
    qDebug() << "ICGearApp Process Starting...";
    qDebug() << "Service: Gear Display for IC (vsomeip Client)";
    qDebug() << "═══════════════════════════════════════════════════════";
    ...
    // Load QML file
    const QUrl url(QStringLiteral("qrc:/qml/ICGearWidget.qml"));
    ...
    if (!engine.rootObjects().isEmpty()) {
        qDebug() << "QML GUI loaded: ICGearWidget.qml";
    }

    qDebug() << "";
    qDebug() << "ICGearApp is running...";
    qDebug() << "═══════════════════════════════════════════════════════";
