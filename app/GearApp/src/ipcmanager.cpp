...
void IpcManager::processReceivedMessage(const QJsonObject &message)
{
    ...
    if (type == "heartbeat_response" || type == "heartbeat") {
        ...
        // ═══════════════════════════════════════════════════════════
        // IC → HU: Gear Status 수신
        // ═══════════════════════════════════════════════════════════
        // IC에서 전송된 gear status를 수신하여 HU 내부로 전파
        // 이 시그널을 GearManager가 수신하여 상태 동기화
        // 
        // vsomeip 전환 시: 이 로직은 vsomeip event handler로 이동
        // ═══════════════════════════════════════════════════════════
        QString gear = message["gear"].toString();
        qDebug() << "IpcManager: [IC → HU] Gear status received from IC:" << gear;
        
        // GearManager로 gear status 전달
        emit gearStatusReceivedFromIC(gear);
    }
    else {
