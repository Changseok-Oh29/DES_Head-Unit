...
int main(int argc, char *argv[])
{
    ...
    if (qgetenv("VSOMEIP_APPLICATION_NAME").isEmpty()) {
        qputenv("VSOMEIP_APPLICATION_NAME", "BatteryApp");
    }

    if (qgetenv("VSOMEIP_CONFIGURATION").isEmpty()) {
        qputenv("VSOMEIP_CONFIGURATION", "/etc/vsomeip/vsomeip_battery.json");
    }
    ...
    app.setApplicationName("BatteryApp");
    app.setApplicationVersion("1.0");
    app.setOrganizationName("SEA-ME");

    qDebug() << "═══════════════════════════════════════════════════════";
    qDebug() << "BatteryApp Process Starting...";
    qDebug() << "Service: Battery Display (vsomeip Client)";
    qDebug() << "═══════════════════════════════════════════════════════";
    ...
    // Load QML file
    const QUrl url(QStringLiteral("qrc:/qml/BatteryWidget.qml"));
    ...
    if (!engine.rootObjects().isEmpty()) {
        qDebug() << "QML GUI loaded: BatteryWidget.qml";
    }

    qDebug() << "";
    qDebug() << "BatteryApp is running...";
    qDebug() << "═══════════════════════════════════════════════════════";
